...
Setup also includes a bastion host for private API access.
*/
...
module "bastion-host-1" {
  source                                        = "./modules/azure/bastion"
  subscription_id                               = var.azure_subscription_id
  environment                                   = var.azure_environment
  bastion_host_size                             = var.bastion_host_size
  admin_username                                = var.admin_username
  admin_ssh_pubkey                              = var.admin_ssh_pubkey
  ip_ranges_allowed                             = var.ip_ranges_allowed_to_bastion
  tags                                          = var.tags
  ...
  resource_group_name                           = module.azure-core.resource_group_name
  resource_group_region                         = module.azure-core.resource_group_region
  vnet_name                                     = module.azure-core.vnet_name
  bastion_subnet_id                             = module.azure-core.bastion_subnet_id
}
...
output "bastion_public_ip_address" {
    value = try(module.bastion-host-1.bastion_public_ip_address, null)
}

output "bastion_public_dns_name" {
    value = try(module.bastion-host-1.bastion_public_dns_name, null)
}

output "bastion_nsg_name" {
    value = try(module.bastion-host-1.bastion_nsg_name, null)
}

output "bastion_host_private_ip" {
    value = try(module.bastion-host-1.bastion_host_private_ip, null)
}
